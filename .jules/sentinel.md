...
## 2025-02-27 - [Information Leakage via Unhandled Unique Constraint Violations]
**Vulnerability:** Raw `sqlx::Error` values were being returned from `create_user`, `create_api_key`, and `create_model_alias` when a unique constraint was violated. These errors eventually propagated as 500 Internal Server Errors, potentially leaking internal database schema details (e.g., table/column/constraint names) to clients.
**Learning:** In Rust with `sqlx`, raw database errors from operations like `fetch_one` or `execute` must be explicitly mapped to domain errors. Using `?` directly on `fetch_one` bypasses this handling. We need to check `is_unique_violation()` to return a safe 409 Conflict domain error (`DbError::UniqueViolation`).
**Prevention:** Always use `.map_err()` to inspect `sqlx::Error`s for specific constraint violations before propagating them, especially on insertion or update operations where unique constraints exist.
