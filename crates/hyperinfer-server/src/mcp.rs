...
mod tests {
    ...
    #[test]
    fn test_validate_jwt_missing_exp_fails() {
        use jsonwebtoken::{encode, EncodingKey, Header};
        ...
        let res = validate_jwt(&token, secret);
        assert!(res.is_err());
        // Verify it's a validation error related to missing claim
        if let Err(e) = res {
            assert!(format!("{:?}", e).contains("MissingRequiredClaim"));
        }
    }

    #[test]
    fn test_validate_jwt_expired_fails() {
        use jsonwebtoken::{encode, EncodingKey, Header};
        ...
        assert!(res.is_err());
        if let Err(e) = res {
            assert!(format!("{:?}", e).contains("ExpiredSignature"));
        }
    }
