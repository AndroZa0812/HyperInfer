...
use hyperinfer_core::{Config, ChatRequest, ChatResponse, HyperInferError, rate_limiting::RateLimiter, types::Provider};
use std::sync::Arc;
...
impl HyperInferClient {
    pub async fn new(redis_url: &str, config: Config) -> Result<Self, HyperInferError> {
        ...
        let rate_limiter = RateLimiter::new(Some(redis_url)).await.map_err(|e| HyperInferError::Config(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())))?;
        let telemetry = Telemetry::new(redis_url).await.map_err(|e| HyperInferError::Config(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())))?;
    ...
    pub async fn chat(&self, key: &str, request: ChatRequest) -> Result<ChatResponse, HyperInferError> {
        ...
        if !allowed.unwrap() {
            return Err(HyperInferError::RateLimit("Rate limit exceeded".to_string()));
        }
        ...
        let response = match provider {
            Provider::OpenAI => {
                self.http_caller.call_openai(&model, &api_key, &request).await?
            }
            Provider::Anthropic => {
                self.http_caller.call_anthropic(&model, &api_key, &request).await?
            }
            _ => {
        ...
        let _ = self.rate_limiter.record_usage(key, total_tokens as u64).await;
